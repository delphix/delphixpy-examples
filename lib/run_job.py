    ...
    if engine == 'all':
        ...
        try:
            ...
            dx_logging.print_exception(
                f'Error encountered in run_job():{err}')
    elif engine == 'default':
        try:
    ...
    while engine_running_jobs:
        for j in engine_running_jobs:
            ...
            if job_obj.job_state in ['COMPLETED']:
                ...
                dx_logging.print_info(f'Engine: {engine["hostname"]}: {job_obj.reference} is 100% COMPLETE')
            elif job_obj.job_state in ['CANCELED', 'FAILED']:
                engine_running_jobs.remove(j)
                dx_logging.print_info(f'Engine: {engine["hostname"]}: {job_obj.reference} was CANCELLED or FAILED due to an error')
                #raise dlpx_exceptions.DlpxException('Job {job_obj.job_id} {job_obj.job_state}')
            elif job_obj.job_state in 'RUNNING':
                dx_logging.print_info(f'Engine: {engine["hostname"]}: {job_obj.reference} is RUNNING and {job_obj.percent_complete}% complete ')
        if dx_obj.jobs:
            time.sleep(poll)
