...
class GetReferencesTests(unittest.TestCase):
    ...
    def test_find_obj_by_name(self):
        ...
        obj_ref = get_references.find_obj_by_name(
            self.server_obj.server_session, web.environment, self.env_name
        )
        self.assertIsInstance(
            obj_ref, web.objects.UnixHostEnvironment.UnixHostEnvironment
        )
    ...
    def test_find_source_by_db_name(self):
        ...
        self.assertIsInstance(
            src_obj, web.objects.OracleVirtualSource.OracleVirtualSource
        )

    def test_find_obj_name(self):
        print("TEST - Find object name from reference")
        obj_name = get_references.find_obj_name(
            self.server_obj.server_session, web.database, self.db_reference
        )
    ...
    def test_find_db_repo(self):
        ...
        self.assertIsInstance(db_repo_name, web.objects.OracleInstall.OracleInstall)

    def test_find_sourceconfig(self):
        print("TEST - Find Sourceconfig")
        src_name = get_references.find_sourceconfig(
            self.server_obj.server_session, self.sourceconfig_name, self.env_ref
        )
        self.assertIsInstance(src_name, web.objects.OracleSIConfig.OracleSIConfig)
