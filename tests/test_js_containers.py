...
class JetStreamContainerTests(unittest.TestCase):
    ...
    def test_adds_removes_js_user(self):
        owner_name = 'jsuser'
        js_container.add_owner(self.server_obj, owner_name,
                               self.container_name)
        self.assertIn(owner_name, sys.stdout.getvalue().strip())

        js_container.remove_owner(self.server_obj, owner_name,
                                  self.container_name)
        self.assertIn(owner_name, sys.stdout.getvalue().strip())
    ...
    def test_restore_js_container_to_bookmark(self):
        js_container.restore_container(self.server_obj, self.container_name,
                                       self.bookmark_name)
        self.assertIn(self.container_name, sys.stdout.getvalue().strip())
