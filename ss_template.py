...
Usage:
  ss_template.py (--create_template <name> --database <name> | --list_templates | --delete_template <name>)
...
from delphixpy.v1_10_2.web.vo import JSDataTemplateCreateParameters
from delphixpy.v1_10_2.web.vo import JSDataSourceCreateParameters
...
def create_template(dlpx_obj, template_name, database_name):
    ...
    ss_template_params = SSDataTemplateCreateParameters()
    ...
    try:
        ...
        ss_template_params.type = 'JSDataTemplateCreateParameters'
        template.create(dlpx_obj.server_session, ss_template_params)
            ...
            while (len(dlpx_obj.jobs) > 0 or len(thingstodo) > 0):
                ...
                for j in dlpx_obj.jobs.keys():
                    job_obj = job.get(dlpx_obj.server_session,
                                      dlpx_obj.jobs[j])
                    print_debug(job_obj)
                    print_info('{}: Provisioning SS Template: {}'.format(
