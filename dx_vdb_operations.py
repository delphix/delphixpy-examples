...
"""Start / stop a VDB
Usage:
  dx_vdb_operations.py --vdb <name>
                  (--stop | --start | --enable | --disable)
                  [-d <identifier> | --engine <identifier> | --all]
                  [--debug] [--parallel <n>] [--poll <n>]
                  [--config <path_to_file>] [--logdir <path_to_file>]
  dx_vdb_operations.py -h | --help | -v | --version
Start or stop a VDB

Examples:
  dx_vdb_operations.py -d landsharkengine --vdb testvdb --stop

  dx_vdb_operations.py --vdb --start
...
def vdb_operation(engine, server, jobs, vdb_name, operation):
    ...
    try:
        ...
        raise dlpxExceptionHandler('An error occurred while performing ' +
