...
from delphixpy.v1_8_2.exceptions import HttpError, JobError
from delphixpy.v1_8_2 import job_context
from delphixpy.v1_8_2.web import database, environment, group, job, source, user
from delphixpy.v1_8_2.web.vo import ASESpecificBackupSyncParameters, ASENewBackupSyncParameters, ASELatestBackupSyncParameters, MSSqlSyncParameters

def ase_latest_backup_sync_parameters():
    obj = ASELatestBackupSyncParameters()
...
def snapshot_database(engine, server, jobs, source_obj, container_obj, obj_type=None):
    """
    This function 
    ...
    if source_obj.reference != None :
        ...
        #Ensure the source is enabled. We can't snapshot disabled databases.
            ...
            print_info(engine["hostname"] + ": Syncing " + container_obj.name )
            print_debug(engine["hostname"] + ": Type: " + source_obj.type )
            print_debug(engine["hostname"] + ": " +source_obj.type)
            #If the database is a dSource and a MSSQL type, we need to tell Delphix how we want to sync the database...
            #Delphix will just ignore the extra parameters if it is a VDB, so we will omit any extra code to check
            if (source_obj.type == "MSSqlLinkedSource"):
                sync_params = MSSqlSyncParameters()
                #From last backup?
                if usebackup == True:
                    sync_params.load_from_backup = True
                    print_info(engine["hostname"] + ": MSSQL database. Creating snapshot of " + container_obj.name + " from Latest Full backup.")
                #Or take a new backup?
                else:
                    sync_params.load_from_backup = False
                    print_info(engine["hostname"] + ": MSSQL database. Creating snapshot of " + container_obj.name + " from New Full backup.")
                print_debug(engine["hostname"] + ": " +str(sync_params))
                #Sync it
                database.sync(server, container_obj.reference, sync_params)
            ...
            #Delphix will just ignore the extra parameters if it is a VDB, so we will omit any extra code to check
                ...
                #Or take a new backup?
                    ...
                    sync_params = ASENewBackupSyncParameters()
                    print_info(engine["hostname"] + ": ASE database. Creating snapshot of " + container_obj.name + " from Full backup.")
                print_debug(engine["hostname"] + ": " +str(sync_params))
                #Sync it
                database.sync(server, container_obj.reference, sync_params)
            #If it isn't MSSQL or ASE, Delphix can just go ahead and sync the database
            else:
                #Sync it
                database.sync(server, container_obj.reference)
