...
from delphixpy.v1_8_2.web import network, system, user, service
from delphixpy.v1_8_2.web.vo import PasswordCredential, User, NetworkInterface, \
...
def help():
    print( basename(__file__)+ ' [-e <engine ip>] [-o <old sysadmin '
        'password] [-p <new sysadmin password]')
    print('Script requires five parameters, the IP of the Delphix Engine, '
        'the default\n gateway, a comma delimited string of DNS servers, the '
        'initial sysadmin \n password to connect with, and the new sysadmin '
        'password you want to use')
    print("-h - Prints this message")
    print('-e <Delphix Engine IP>  - The IP to use to connect to the Delphix '
        'Engine. \nEngine must be up, unconfigured, and console screen must be '
        'green')
    print('-p <new sysadmin password>  - will set the sysadmin user to this '
        'password')
    print('-n <new Delphix Engine CIDR>  - will set the Delphix Engine to this '
        'IP address \n(i.e. 10.0.1.10/24)')
    print('-g <default gateway> - will set the default gateway to point to '
        'this \nIP address')
    print('-d <dns servers> - comma delimited string of dns servers to use \n'
...
def logging_est():
    ...
    '''
    global debug
    logging.basicConfig(filename='landshark_setup.log',\
...
def main(argv):
    try:
        ...
        try:
            print_debug("Changing the IP address. This operation can take up to 60 seconds to complete")
