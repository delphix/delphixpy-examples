...
def create_branch(dlpx_obj, branch_name, container_name, template_name=None,
    ...
    if bookmark_name:
        ...
        ss_branch.timeline_point_parameters.bookmark = \
    ...
    dx_logging.print_info(f'Self Service Branch {branch_name} was created'
                          f' successfully.\n')
    ...
    try:
        ...
        for ss_branch in ss_branches:
            js_end_time = selfservice.operation.get(
                dlpx_obj.server_session, ss_branch.first_operation).end_time
            ...
            dx_logging.print_info(f'{ss_branch._name[0]} {js_data_layout},'
                                  f'{ss_branch.reference}, {js_end_time}')
    except dlpx_exceptions.DlpxException as err:
        raise (f'ERROR: Self Service Branches could not be listed. The error '
               f'was:\n{err}')
...
def update_branch(dlpx_obj, branch_name):
    ...
    try:
        ...
        selfservice.branch.update(dlpx_obj.server_session,
                                  branch_obj.reference, ss_branch_obj)
    except (dlpx_exceptions.DlpxException, exceptions.HttpError,
            exceptions.RequestError) as err:
        raise dlpx_exceptions.DlpxException(f'ERROR: The branch could not be '
                                            f'updated. The error was:{err}')
    dx_logging.print_info(f'The branch {branch_name} was updated '
                          f'successfully.\n')
...
def activate_branch(dlpx_obj, branch_name):
    ...
    try:
        branch_obj = get_references.find_obj_by_name(
            dlpx_obj.server_session, selfservice.branch, branch_name)
        selfservice.branch.activate(dlpx_obj.server_session,
                                    branch_obj.reference)
        dlpx_obj.dlpx_ddps['engine_name'] = dlpx_obj.server_session.last_job
    except exceptions.RequestError as err:
        raise dlpx_exceptions.DlpxException(
            f'ERROR: An error occurred activating the {branch_name}:\n{err}')
    dx_logging.print_info(f'The branch {branch_name} was activated '
                          f'successfully.\n')
...
def delete_branch(dlpx_obj, branch_name):
    ...
    try:
        branch_obj = get_references.find_obj_by_name(
            dlpx_obj.server_session, selfservice.branch, branch_name)
        selfservice.branch.delete(dlpx_obj.server_session,
                                  branch_obj.reference)
    except (dlpx_exceptions.DlpxException, exceptions.HttpError,
            exceptions.RequestError) as err:
        raise dlpx_exceptions.DlpxException(
